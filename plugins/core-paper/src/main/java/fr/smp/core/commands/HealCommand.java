...
public class HealCommand implements CommandExecutor, TabCompleter {
    ...
    public HealCommand(SMPCore plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (target != sender) {
            sender.sendMessage(Msg.ok("<gray><aqua>" + target.getName() + "</aqua> a été soigné(e).</gray>"));
        }
        target.sendMessage(Msg.ok("<gray>Tu as été <green>soigné(e)</green>.</gray>"));
        return true;
    }
