...
public final class NpcNms {
    ...
    static {
        ...
        try {
            ...
            try {
                ...
                pTeleportEntity = Class.forName("net.minecraft.network.protocol.game.ClientboundEntityPositionSyncPacket");
            }
            ...
            // ClientboundTeleportEntityPacket(Entity) — used post-move
            try {
                ctTeleport = pTeleportEntity.getConstructor(entity);
            } catch (NoSuchMethodException e) {
                // Older signature ClientboundTeleportEntityPacket(int id, PositionMoveRotation, ...)
                ctTeleport = null;
            }

            // Player.DATA_PLAYER_MODE_CUSTOMISATION
            fDataCust = playerNms.getDeclaredField("DATA_PLAYER_MODE_CUSTOMISATION");
            fDataCust.setAccessible(true);
    ...
    public static Object createFakePlayer(World world, Location loc, Object gameProfile) throws Exception {
        ...
        // Active toutes les couches de skin (cape, veste, manches, pants, chapeau)
    ...
    public static Object buildTeleportPacket(Object nmsPlayer) throws Exception {
        if (CT_TELEPORT_ENTITY == null) return null;
